...
function GetLibcateg($id_categ)
{
	...
	//echo $getcateglib;exit();
	$execateglib=mysql_query($getcateglib) or die(mysql_error());
	$libcateg=mysql_fetch_object($execateglib);
...
{
	$getrublib="SELECT libelle FROM rubrique WHERE id='".$id_rbq."'";
	//echo $getcateglib;exit();
	$exerublib=mysql_query($getrublib) or die(mysql_error());
	$librub=mysql_fetch_object($exerublib);
	return $librub->libelle;
...
{
	$getsigldenoref="SELECT sigle, entreprise FROM entreprise WHERE Id_ese='".$id_ste."' ";
	//echo $getrublib;exit();
	$exegetsigldenoref=mysql_query($getsigldenoref) or die(mysql_error());
	while($sigldenoref=mysql_fetch_object($exegetsigldenoref)){
	return $sigldenoref->sigle.'-'.$sigldenoref->entreprise;}
	@mysql_free_result($exegetsigldenoref);
}
###

function Getimage_ssrub($id_srub)
{
	$getsigldenoref="SELECT * FROM pub2 WHERE Id_sr = '".$id_srub."' limit 1;";
	//echo $getrublib;exit();
	$exegetsigldenoref=mysql_query($getsigldenoref) or die(mysql_error());
	while($sigldenoref=mysql_fetch_array($exegetsigldenoref)){
	return $sigldenoref['images'].'--'.$sigldenoref['date'];}
	@mysql_free_result($exegetsigldenoref);
}
###
