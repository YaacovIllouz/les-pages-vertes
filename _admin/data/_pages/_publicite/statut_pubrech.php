...
{
	document.location.href = "accueil.php?page=?.liste_pubrech";
}
//-->
</script>
...
$date_mod = date('Y-m-d H:s:i');
		...
		if($result['flag_pub']==1)
		{
		$sql2 = "UPDATE pub SET flag_pub='0', login_user='".$_SESSION['login_user']."' WHERE id_pub='".$id."'";
		  //exécution de la requête:
		  $requete2 = mysql_query( $sql2) or die(mysql_error());//mise à jour d'un user après deconnexion
			echo "<script>redirection();</script>";
			}
		else if($result['flag_pub']==0)
		{
		$sql2 = "UPDATE pub SET flag_pub='1', login_user='".$_SESSION['login_user']."' WHERE id_pub='".$id."'";
		  //exécution de la requête:
		  $requete2 = mysql_query( $sql2) or die(mysql_error());//mise à jour d'un user après deconnexion
			echo "<script>redirection();</script>";	
			}
