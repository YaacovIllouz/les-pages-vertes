<?php
//pub gauche
$pub_gauche = $db->query("SELECT image FROM pub WHERE Id_ese = '".$_GET['id']."' ORDER BY Id DESC ")->fetch();
/*if(!$pub_gauche){
...
<div class="col-md-4">
    ...
    <?php if($pub_gauche){ ?>
        <a href="<?= $ese['web']; ?>" target="_blank">
            <img src="<?= $pub_gauche['image']; ?>" class="img-responsive img-rounded"  style="max-height: 948px;height:948px;width:263px;" />
        </a>
    <?php } ?>
    </div>
</div><!--col-md-3"-->
...
<div class="col-md-8"  style="min-height:100px; border-radius:3px; border:1px solid <?=$ese['color'];?>;">
    <div class="box" style="padding:5px; font-size:15px; margin-top:15px;">
        <div class="row">
            <div class="col-md-3">
                <img alt="" src="<?php echo $ese['image']; ?>" class="img-responsive"/>
            </div><!--col-md-6"-->

            <div class="col-md-9">
                <h4 style="font-weight:bold; font-size:20px;"><b><?= minToMajSansAccent($ese['sigle']); ?></b></h4>
                <h5 style="font-weight:bold; font-size:18px;"><b><?= minToMajSansAccent($ese['entreprise']); ?></b></h5>
...
<br/>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
               <div class="titre_droit4" style="text-align:center; border-radius:3px;">
                   <a href="index.php?page=contact" style="color:#FFF;">Send a message</a>
               </div>
            </div><!--col-md-4-->
        </div>
        <br/>
