...
{
	document.location.href = "accueil.php?page=?.liste_ancenact";
}
//-->
</script>
...
$date_mod = date('Y-m-d H:s:i');
		...
		{
		$sql2 = "UPDATE annonce SET flag_anc='0', valid_login_user='".$_SESSION['login_user']."', date_mod_ref='".$date_mod."' WHERE id_ste='".$id."'";
		  //exécution de la requête:
		  $requete2 = mysql_query( $sql2) or die(mysql_error());//mise à jour d'un user après deconnexion
			echo "<script>redirection();</script>";
			}
		else if($result['flag_anc']==0)
		{
		$sql2 = "UPDATE reference SET flag_anc='1', valid_login_user='".$_SESSION['login_user']."', date_mod_ref='".$date_mod."' WHERE id_ste='".$id."'";
		  //exécution de la requête:
		  $requete2 = mysql_query( $sql2) or die(mysql_error());//mise à jour d'un user après deconnexion
			echo "<script>redirection();</script>";	
			}
