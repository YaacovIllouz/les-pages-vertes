...
if($result_default['flag_default']==0)
	...
	$msge.='<meta http-equiv="refresh" content="2; url=accueil.php?page=?.liste_pubrech" />';		 
	}
	else {
		$msge='<font class="refuse">SUPPRESSION REFUSEE.<br>(Cette image est celle affich&eacute;e par defaut)</font>';
	$msge.='<meta http-equiv="refresh" content="5; url=accueil.php?page=?.liste_pubrech" />';
...
$query_pag_data = "SELECT * from $table WHERE emplct_pub = 'DROITE' ORDER BY date_crea_pub ASC";
//echo $query_pag_data;
...
<script language=javascript>
   function ConfirmMessage( identifiant ) 
   {
	   ...
	   { // Clic sur OK
           document.location.href = "accueil.php?page=?.liste_pubrech&com="+identifiant ;
       }
   }
</script>
...
<script language=javascript>
   ...
   {
       // Clic sur OK
           document.location.href = "accueil.php?page=?.statut_pubrech_default&com="+identifiant ;
...
<div style="margin:auto">
  <table width="100%" border="0">
      <tr>
        <td width="35%" align="left"><h3 class="titre_page">PUB DE DROITE</h3></td>
        <td width="43%" align="center"><?php echo $msge;?></td>
        <td width="22%" align="right"><div align="right" class="text_bouton"><a href="accueil.php?page=?.add_pub" class="ajout">[Ajouter une affiche]</a></div></td>
      </tr>
    </table>   
    </div>
<br />
		...
		?> 
								  <tr>
                                      ...
                                      <td align="center"><img src="../../_affiches/pub_rech/<?php echo $row['img_pub'];?>" width="50" height="40"></td>
									  <td style="width:60px"><?php echo $row['lib_pub'];?></td>
								    <td><?php echo date("d/m/Y H:s:i",strtotime($row['date_crea_pub']));?></td>
								    <td><?php echo 'Du '.date("d/m/Y",strtotime($row['date_deb_pub'])).' au '.date("d/m/Y",strtotime($row['date_fin_crea']));?></td>
									  <td align="center">
&nbsp;&nbsp;&nbsp;<a href="#" onClick="lien_default(<?php echo $row['id_pub'];?>)"><input type="radio" name="statut" <?php if($row['flag_default']==1) echo "checked";?> title="Image par d&eacute;faut"></a>&nbsp;&nbsp;&nbsp;
										  ...
										  <a href="accueil.php?page=?.statut_pubrech&id=<?php echo $row['id_pub'];?>"><?php if($row['flag_pub']==1){echo '<img src="img/unlocked.png" title="Cliquer pour v&eacute;rouiller">';} else {echo '<img src="img/locked_.png" title="Cliquer pour d&eacute;v&eacute;rouiller">' ;}?></a>&nbsp;&nbsp;&nbsp;
