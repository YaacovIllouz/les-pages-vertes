<?php
//if($_GET['id']) echo "Bonsoir : ".$_GET['id'];
...
<div class="col-md-4">
    <div style="min-height:100px; padding:0px; height:950px; width:265px; border-radius:3px 3px 3px 3px; border:1px solid <?= $color['color'];?>;">
        <?php 
        if($pub){ 
            echo '<img src="'.$pub['images'].'" class="img-responsive" style="width:auto;" alt="logo" />';
        }
        ?> 
    </div>
</div><!--col-md-3"-->
...
<div class="col-md-8">
     ...
     <div class="box">
        <br />
		 ...
		 if(($ese['entreprise']!="") && ($ese['sigle']!="")) {
			 $info_ese = format_url($ese['sigle']).'-'.format_url($ese['entreprise']);
			 } 
			 else if(($ese['entreprise']!="") && ($ese['sigle']=="")) {
			 $info_ese = format_url($ese['entreprise']);
			 }else if(($ese['entreprise']=="") && ($ese['sigle']!="")) {
			 $info_ese = format_url($ese['sigle']);
			 }
        ...
        <div style="min-height:100px; border:1px solid <?= $color['color'];?>; margin-bottom:20px; border-radius:10px;">
            ...
            <div class="col-md-9">
                <p style="text-transform:uppercase; font-weight:bold; margin-top:10px;">
                    <a href="entreprise-<?php echo $ese['Id_ese'].'&'.$info_ese.'-'.format_url(GetLibcateg($ese['Id_rubrique'])).'-'.format_url(GetLibRub($ese['Id_sous_rubrique'])).'-annuaire-agriculture-lespagesvertes'; ?>">
                        <span style="color:#000; font-size:20px;"><b><?php echo $ese['sigle']; ?></b></span><br />
                        <span style="color:#000; font-size:17px;"><b><?php echo $ese['entreprise']; ?></b></span>
                    </a>
                </p>
                <p>
               ...
               <p><b>Cel. &nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;<span style="color:#000;"><?php echo $ese['cel1']; ?></span></b></p>
            ...
            <div class="" style="background:<?= $color['color'];?>; border:1px solid <?= $color['color'];?>;
                ...
                <p style="float:right;">
                    <a href="entreprise-<?php echo $ese['Id_ese'].'&'.$info_ese.'-'.format_url(GetLibcateg($ese['Id_rubrique'])).'-'.format_url(GetLibRub($ese['Id_sous_rubrique'])).'-annuaire-agriculture-lespagesvertes'; ?>">
                        <b style="color:#FFF;"><i class="fa fa-arrow-right"></i> Plus de d&eacute;tails</b>
                    </a>
                </p>
            </div> 
        </div>
    ...
    <div style="text-align: center;">   <!-- DC Pagination:A1 Start -->
        ...
        <?php 
		//rubrique et sous_rubrique par page
		
		    //pagination
		
		$page = "rubrique-".$id.'&'.format_url(GetLibcateg($ese['Id_rubrique'])).'-'.format_url(GetLibRub($ese['Id_sous_rubrique'])).'-annuaire-agriculture-lespagesvertes';
		$page  = $page."&current_page=";
		
		$page1 = "rubrique-".$id.'&'.format_url(GetLibcateg($ese['Id_rubrique'])).'-'.format_url(GetLibRub($ese['Id_sous_rubrique'])).'-annuaire-agriculture-lespagesvertes';
        ...
        // nombre total de pages
          ...
          echo '<li><a href="'.$page1.'" class="first" title="D&eacute;but">&lt;&lt;</a></li>';

          // page précédente
          if($courant > 1)
            echo '<li><a href="'.$page.($courant-1).'" class="previous" title="Pr&eacute;c.">&lt;</a></li>';
          ...
          // page suivante
          if($courant < $nb_pages)
        echo '<li><a href="'.$page.($courant + 1).'" class="next" title="Suiv.">&gt;</a></li>';

          if($derniere && $courant + $apres < $nb_pages)
          echo '<li><a href="'.$page.$nb_pages.'" class="last" title="Fin">&gt;&gt;</a></li>';
        ?>
        </ul>
    </div>
</div><!--col-md-8"-->
