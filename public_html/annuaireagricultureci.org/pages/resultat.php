<?php
/*if($_GET['recherche']) { 
echo 'mot recherché : '.$_GET['recherche'];} 
else { echo 'pas de mot trouvé';}*/
...
?>
    ...
    <div class="col-md-4">
       <?php
       $pub = $db->query("SELECT image FROM pub_site WHERE position = 'Recherche' AND etat=1 limit 1;")->fetch();
       ?>
  ...
  if(($row_rs_search['entreprise']!="") && ($row_rs_search['sigle']!="")) {
			 $info_ese = format_url($row_rs_search['sigle']).'-'.format_url($row_rs_search['entreprise']);
			 } 
			 else if(($row_rs_search['entreprise']!="") && ($row_rs_search['sigle']=="")) {
			 $info_ese = format_url($row_rs_search['entreprise']);
			 }else if(($row_rs_search['entreprise']=="") && ($row_rs_search['sigle']!="")) {
			 $info_ese = format_url($row_rs_search['sigle']);
			 }
  ...
  ?>
       <div class="row" style="min-height:100px; border:1px solid <?= $row_rs_search['color']; ?>; margin-bottom:10px; border-radius:10px;">
          ...
          <div class="col-md-9">
               <b style="color:#060;">
                   <a href="entreprise-<?php echo $row_rs_search['Id_ese'].'&'.$info_ese.'-'.format_url(GetLibcateg($row_rs_search['Id_rubrique'])).'-'.format_url(GetLibRub($row_rs_search['Id_sous_rubrique'])).'-annuaire-agriculture-lespagesvertes'; ?>">
                       <span style="font-weight:bold; color:#000; font-size:20px;"><?php echo $row_rs_search['sigle']; ?></span><br />
                       <span style="font-weight:bold; color:#000; font-size:15px;"><?php echo mb_strtoupper($row_rs_search['entreprise']); ?></span>
                   </a>
               </b>
                <br />
                <p>
            ...
            <img src="<?= $row_rs_search['image']; ?>" class="img-responsive" alt="Logo"  width="100px" height="110px" style="max-height:105px; max-width:98px" />
           </div>
