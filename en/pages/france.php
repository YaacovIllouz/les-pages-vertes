...
if(isset($_GET['go']) || isset($_GET['id_region'])) {
    ...
    if(isset($_GET['go'])) {
        ...
        $requete = "SELECT Id, rubrique FROM rubrique ORDER BY rubrique";
    } else if(isset($_GET['id_region'])) {
        $id = htmlentities(intval($_GET['id_region']));
        // requête qui récupère les départements selon la région
        $requete = "SELECT Id, rubrique FROM  sous_rubrique WHERE Id_rubrique = ". $id ." ORDER BY rubrique";
    }
     
    // connexion à la base de données
    try {
        $bdd = new PDO('mysql:host=91.216.107.161;dbname=lespa580653', 'lespa580653', 'fJGGACgQ');
    } catch(Exception $e) {
